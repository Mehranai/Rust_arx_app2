...
pub fn is_nft_transfer(receipt: &TransactionReceipt) -> bool {
    let erc721_sig = H256::from(keccak256("Transfer(address,address,uint256)"));
...
fn collect_token_flows(receipt: &TransactionReceipt, user: Address) -> TokenFlow {
...
fn detect_lp_add(flow: &TokenFlow) -> Option<LiquidityPoolDetails> {
    // LP mint → received
    if flow.received.len() != 1 || flow.sent.len() != 2 {
        return None;
    }

    let (&lp_token, &lp_amount) = flow.received.iter().next().unwrap();
...
fn detect_lp_remove(flow: &TokenFlow) -> Option<LiquidityPoolDetails> {
    // LP burn → sent
    if flow.sent.len() != 1 || flow.received.len() != 2 {
        return None;
    }

    let (&lp_token, &lp_amount) = flow.sent.iter().next().unwrap();
...
pub fn is_liquidity_pool(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn find_bridge_event<'a>(receipt: &'a TransactionReceipt) -> Option<&'a ethers::types::Log> {
    receipt
        .logs
        .iter()
        .find(|log| is_bridge_event(log.topics[0]))
}
...
pub fn extract_bridge_details(
    ...
    let (asset, amount) = extract_bridge_asset(tx, receipt, user, bridge_contract)?;
...
fn collect_balance_deltas(
    ...
    for log in &receipt.logs {
        ...
        if log.topics[0] == weth_withdraw_sig() {
            let amount = U256::from_big_endian(&log.data.0);
            *delta.received.entry(Asset::Eth).or_insert(U256::zero()) += amount;
        }

        // -------- WETH wrap --------
        if log.topics[0] == weth_deposit_sig() {
            let amount = U256::from_big_endian(&log.data.0);
            *delta.sent.entry(Asset::Eth).or_insert(U256::zero()) += amount;
        }
    }
...
fn normalize_deltas(delta: BalanceDelta) -> (HashMap<Asset, U256>, HashMap<Asset, U256>) {
...
fn is_real_swap(sent: &HashMap<Asset, U256>, received: &HashMap<Asset, U256>) -> bool {
...
pub fn extract_swap_details(tx: &Transaction, receipt: &TransactionReceipt) -> Option<SwapDetails> {
    let user = tx.from;
...
fn extract_erc20_transfers(receipt: &TransactionReceipt) -> Vec<ERC20TransferDetails> {
    let mut out = Vec::new();
...
pub fn is_erc20_transfer(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn has_weth_wrap_or_unwrap(receipt: &TransactionReceipt) -> bool {
...
pub fn is_eth_transfer(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn extract_approve_events(receipt: &TransactionReceipt, user: Address) -> Vec<ApproveDetails> {
    let mut out = Vec::new();
    let sig = approve_sig();
...
fn allowance_used_in_same_tx(receipt: &TransactionReceipt, user: Address, token: Address) -> bool {
    let transfer_sig = H256::from(keccak256("Transfer(address,address,uint256)"));
...
pub fn is_approve(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn stake_method_selectors() -> Vec<[u8; 4]> {
    vec![
        ...
        keccak256("deposit(address,uint256,address,uint16)")[0..4]
            .try_into()
            .unwrap(),
...
fn extract_stake_flows(
    ...
    if tx.value > U256::zero() {
        return Some((Asset::Eth, tx.value, None));
    }
    ...
    Some((sent_asset, sent_amount, received))
}
...
pub fn extract_stake_details(
    ...
    let (sent_asset, sent_amount, received) = extract_stake_flows(tx, receipt, user)?;
...
pub fn is_stake(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn withdraw_method_selectors() -> Vec<[u8; 4]> {
    vec![
        ...
        keccak256("withdraw(address,uint256)")[0..4]
            .try_into()
            .unwrap(),
...
fn extract_withdraw_flows(
    ...
    Some((received_asset, received_amount, burned))
}
pub fn extract_withdraw_details(
    ...
    let (received_asset, received_amount, burned) = extract_withdraw_flows(tx, receipt, user)?;
...
pub fn is_withdraw(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
...
fn deposit_method_selectors() -> Vec<[u8; 4]> {
    vec![
        keccak256("deposit(uint256)")[0..4].try_into().unwrap(),
        keccak256("deposit(address,uint256)")[0..4]
            .try_into()
            .unwrap(),
        keccak256("deposit(address,uint256,address)")[0..4]
            .try_into()
            .unwrap(),
...
pub fn extract_deposit_details(
    ...
    let (asset, amount, received) = extract_deposit_flows(tx, receipt, user)?;
...
pub fn is_deposit(tx: &Transaction, receipt: &TransactionReceipt) -> bool {
